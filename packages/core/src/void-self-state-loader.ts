...
import { type VoidSelfStateTypedProjection } from "./void-self-state-projection";
...
export async function loadVoidSelfState(
  ...
  let typedProjection: VoidSelfStateTypedProjection;
  try {
    typedProjection = await loadVoidSelfStateTypedDocuments({ canonicalPath: statePath });
  } catch (error) {
  ...
  return {
    ...
    summary: renderTypedVoidSelfStateSummary(typedProjection, options),
    projection,
  };
}

function renderTypedVoidSelfStateSummary(
