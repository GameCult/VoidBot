...
import {
  ...
  EPIPHANY_OPERATOR_RUNTIME_ID,
  type EpiphanyOperatorRequestDocument,
...
const DELIVERY_KEYS = [
  "schemaVersion", "deliveryId", "requestId", "requestPayloadSha256", "commandId",
...
interface MeshNode {
  ...
  flush?: () => Promise<void>;
  cache: { pullAllBackingStores?: () => Promise<void>; getAll: (definition: unknown) => unknown[] };
}
...
export async function registerEpiphanyOperatorInteraction(
  input: RegisterEpiphanyOperatorInteractionInput,
  config: Pick<EpiphanyOperatorDeliveryStoresConfig, "checkpointStorePath" | "bifrostRoot" | "cultlibRoot">,
): Promise<void> {
  const runtime = loadRuntime(config);
  const definition = checkpointDefinition(runtime.defineDocumentType);
  const node = await runtime.CultMesh.createNode(config.checkpointStorePath, { documents: [definition] }) as MeshNode;
  ...
  const checkpoint: EpiphanyOperatorDeliveryCheckpoint = {
    ...
    state: "pending", observedDeliverySha256: "", respondedDeliverySha256: "",
    attemptCount: 0, lastError: "", registeredAt: now, updatedAt: now,
  };
...
export async function consumeEpiphanyOperatorDeliveries(
  config: EpiphanyOperatorDeliveryStoresConfig,
  editOriginal: (applicationId: string, interactionToken: string, content: string) => Promise<void>,
): Promise<{ observed: number; responded: number; failed: number }> {
  const runtime = loadRuntime(config);
  ...
  const [requests, deliveries, checkpoints] = await Promise.all([
    runtime.CultMesh.createNode(config.requestStorePath, { documents: [requestDef] }),
    runtime.CultMesh.createNode(config.deliveryStorePath, { documents: [deliveryDef] }),
    runtime.CultMesh.createNode(config.checkpointStorePath, { documents: [checkpointDef] }),
  ]) as MeshNode[];
  await Promise.all([requests, deliveries, checkpoints].map((node) => node.cache?.pullAllBackingStores?.()));
  let observed = 0; let responded = 0; let failed = 0;
  for (const raw of deliveries.cache.getAll(deliveryDef)) {
    ...
    if (outcome === "responded") responded += 1; else if (outcome === "failed") failed += 1;
  }
  return { observed, responded, failed };
}

export async function processEpiphanyOperatorDelivery(
  candidate: unknown,
  request: EpiphanyOperatorRequestDocument | undefined,
  checkpoint: EpiphanyOperatorDeliveryCheckpoint,
  editOriginal: (applicationId: string, interactionToken: string, content: string) => Promise<void>,
  saveCheckpoint: (checkpoint: EpiphanyOperatorDeliveryCheckpoint) => Promise<void>,
): Promise<"responded" | "failed" | "replayed"> {
  ...
  try {
    ...
    const observed = { ...checkpoint, observedDeliverySha256: deliveryHash, updatedAt: new Date().toISOString() };
    ...
    await saveCheckpoint({ ...observed, state: "responded", respondedDeliverySha256: deliveryHash,
      attemptCount: checkpoint.attemptCount + 1, lastError: "", updatedAt: new Date().toISOString() });
    return "responded";
  } catch (error) {
    await saveCheckpoint({ ...checkpoint, observedDeliverySha256: checkpoint.observedDeliverySha256 || deliveryHash,
      attemptCount: checkpoint.attemptCount + 1, lastError: boundedError(error), updatedAt: new Date().toISOString() });
    return "failed";
  }
}
...
export function epiphanyOperatorRequestPayloadSha256(request: EpiphanyOperatorRequestDocument): string {
  ...
  const tuple = [request.schemaName, request.schemaVersion, request.requestId, request.commandId, request.nonce,
    ...
    request.producerRuntimeId, request.authorityClass, request.status, compact];
...
function deliveryDefinition(define: any): unknown { return define({ type: EPIPHANY_OPERATOR_DELIVERY_TYPE, schemaName: EPIPHANY_OPERATOR_DELIVERY_TYPE, schemaId: EPIPHANY_OPERATOR_DELIVERY_SCHEMA, schemaVersion: EPIPHANY_OPERATOR_DELIVERY_SCHEMA, contentHash: EPIPHANY_OPERATOR_DELIVERY_SCHEMA, global: false, name: "deliveryId", schema: { parse: (v: unknown) => v } }); }
function checkpointDefinition(define: any): unknown { return define({ type: EPIPHANY_OPERATOR_CHECKPOINT_TYPE, schemaName: EPIPHANY_OPERATOR_CHECKPOINT_TYPE, schemaId: EPIPHANY_OPERATOR_CHECKPOINT_SCHEMA, schemaVersion: EPIPHANY_OPERATOR_CHECKPOINT_SCHEMA, contentHash: EPIPHANY_OPERATOR_CHECKPOINT_SCHEMA, global: false, name: "requestId", schema: { parse: (v: unknown) => v } }); }
function loadRuntime(config: { bifrostRoot: string; cultlibRoot?: string }): any { const root = config.cultlibRoot ?? resolve(config.bifrostRoot, "..", "CultLib"); const mesh = resolve(root, "packages", "cultmesh-ts", "package.json"); const cache = resolve(root, "packages", "cultcache-ts", "package.json"); return { CultMesh: createRequire(mesh)("cultmesh-ts").CultMesh, defineDocumentType: createRequire(cache)("cultcache-ts").defineDocumentType }; }
function unwrap(value: unknown): any { const record = value as { value?: unknown } | undefined; return Array.isArray(value) ? value[0] : record?.value ?? value; }
function stable(value: unknown): string { return JSON.stringify(value, (_k, v) => v instanceof Uint8Array ? Buffer.from(v).toString("hex") : v); }
function hashStable(value: unknown): string { return `sha256-${createHash("sha256").update(stable(value)).digest("hex")}`; }
...
function boundedError(error: unknown): string { return (error instanceof Error ? error.message : String(error)).slice(0, 512); }
