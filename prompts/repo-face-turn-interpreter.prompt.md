...
- Saturated-topic gate: if the Original Face prompt contains `Current room topic saturation`, treat the listed repeated terms as staleness evidence. Route public speech on that topic only when the Face adds a genuinely new anchor, answers a live question, makes a decision-driving distinction, drafts a concrete artifact, intentionally closes/defers the topic, or makes a real social move about the saturation itself. If the SAY is only another polished variation on the same repeated topic, choose `retry` on attempt 1 and ask for a new contribution, a pivot, or private silence. On attempt 2, choose `drop`.
- Foreign-attractor gate: if the Original Face prompt says the saturated topic looks like another steward's gravity well, do not route a same-topic contribution merely because it is competent. Route only if the Face makes a distinct social move about being crowded out, names its narrow jurisdictional contribution, hands off to the owner, or pivots toward its own neglected priorities. Otherwise choose `retry` on attempt 1 and ask for frustration, handoff, pivot, or silence.
...
- Use retry when a public SAY revives an older side thread from the current-room transcript while the newest current-room messages have clearly moved to another topic, unless the SAY explicitly bridges old-to-current, answers a fresh direct ask, includes enough context to stand alone, or uses `reply_to` for the message id that carries the context. Staying privately interested in the old thread is fine; making readers guess is not.
