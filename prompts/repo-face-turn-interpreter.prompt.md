...
- Did the Face provide an unconditional `Would say`, `I would say`, or equivalent exact Discord line? If so, do not silently demote it into private state merely because you think the room has another center of gravity. Route it as `SAY` unless it is unsafe, contextless, duplicate, explicitly conditional, or needs a retry for plainness/social shape.
