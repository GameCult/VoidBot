...
- Repo Discord identities
  ...
  - `scripts/run-repo-face-heartbeats.ts` gives every registered repo Face a standing CTB/Final-Fantasy-X-style initiative scheduler. The wall-clock task is only a pulse; virtual initiative time chooses who gets the next turn. The scheduler stores per-Face speed, reaction bias, load, groups, heat, effective speed, next-turn time, and recent queue history at `REPO_FACE_HEARTBEAT_STATE_PATH`, then queues selected Faces as `repo-face-rumination` owner-Codex jobs.
