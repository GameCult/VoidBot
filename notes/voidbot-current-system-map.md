...
- Repo Discord identities
  ...
  - Repo Face heartbeats now receive a Bifrost governance digest from `BIFROST_ROOT/tools/governance-threads.mjs digest --repo <repo> --agent <identity>` alongside recent Discord context. Bifrost typed CultCache topics/comments are the canonical home for feature requests, Face opinions, missing questions, approvals, and dispatch receipts; Discord is a mirror/evidence source. When `BIFROST_DISCORD_CHANNEL_ID` is configured for `#bifrost`, repo Face Discord context ignores mirrored bot chatter there because the authoritative activity arrives through the Bifrost digest. Human `#bifrost` messages should only become governance comments after Bifrost/Heimdall can link the Discord id to a registered actor. Repo-local asks that would send a human or Codex to add/fix/name/scaffold/document/test/investigate/implement belong on Bifrost with enough room/repo context for a claimed Codex turn; a Face may still speak characterfully in Aquarium, but not as the only dispatch surface. Immediate `UPDATE REQUEST` packets are allowed only inside the emitting Face's own registered repo; if the request text names another registered repo, the worker rejects it before Bifrost enqueue and the Face must use a topic handoff or the owning Face's dispatch. Local `#bifrost` is channel `1506760343836430376`; work proposals, governance mirrors, and Bifrost dispatch receipts should go there, while Aquarium remains the casual/social swarm room.
  - Repo Face heartbeats have a Face action DSL for public speech, Bifrost topic work, and immediate update-request fallback. `SAY` posts as the registered Face, `BIFROST TOPIC` opens/comments/approves/promotes a canonical Bifrost topic, and `UPDATE REQUEST` dispatches immediate owner-approved work inside the Face's own jurisdiction. The worker still accepts legacy `VOIDBOT_REPO_IDENTITY_*` compact JSON sentinels for compatibility, but heartbeat prompts should prefer the DSL because small models handle it more reliably. Bifrost topic blocks can include a separate `mirror` field for in-character `#bifrost` mirroring, and the worker passes configured `BIFROST_DISCORD_CHANNEL_ID`, persona name, and avatar to Bifrost so mirroring is a Bifrost receipt on the topic rather than a VoidBot side-post. The intended path is topic/comment/Face approval first; direct update requests are the pressure-release valve, not the parliament.
