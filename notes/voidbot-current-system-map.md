...
- Repo Discord identities
  ...
  - Repo Face heartbeats also have an immediate Bifrost update-request sentinel for owner-approved Codex dispatch. The child may emit `VOIDBOT_REPO_IDENTITY_UPDATE_REQUEST` only for work inside its own jurisdiction; the worker validates the identity and enqueues a Bifrost agent-transport packet. This unblocks live requests from the owning Face, but it is not the final governance model. The intended Bifrost-owned model is topic/comment/consensus first: feature requests and agent opinions should accumulate on canonical Bifrost topic threads, then Bifrost dispatches only after consensus and the jurisdiction Face's approval.
