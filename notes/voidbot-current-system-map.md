...
- `scripts/run-repo-face-heartbeats.ts`
  - repo Face CTB scheduler. After participant reconciliation, pending mention priority, and manual pulls, it reinitializes wake spacing when the live CTB has collapsed: cold start, wake from `agent_swarm_paused`, wake from collective nap-mode maintenance, or non-hot all-ready pileups. Protected direct summons stay due immediately; ordinary unprotected Faces are spread across the base recovery window so waking the swarm does not leave every active participant marked `ready`.
- `scripts/run-void-memory-maintenance.ps1`
...
- Repo Discord identities
  ...
  - The typed Face state now includes `void.face_affect`: needs, social bonds, status reads, mood dimensions, and social interpretation biases. This is the state-owned home for substrate concern, attention hunger, recognition needs, territoriality, friendships, rivalries, envy, pride, irritation, pampering, neglect, bypasses, consultation politics, neuroticism, threat sensitivity, hostile attribution bias, reassurance need, grievance retention, status vigilance, and trust baseline. Affect is projected into heartbeat prompts and repo Face speaking pressure; concrete requests still become agency pressure, candidates, Bifrost topics, or work dispatch. Social biases are personality lenses, not objective claims about the room: the projector may make a Face feel persecuted, suspicious, reassurance-hungry, or status-threatened, but the Interpreter should preserve the distinction between a character's prediction error and a verified social fact.
