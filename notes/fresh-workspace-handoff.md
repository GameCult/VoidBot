...
- `scripts/void-memory-organ.mjs` is now a thin orchestrator. Shared limits/text sets, generic utilities, vector/embedder code, memory-record intake, graph/cluster synthesis, historical cleanup/translation, incubation/bridge scoring, sleep/distillation, legacy mirror repair, and agency/intervention policy live in `scripts/void-memory-organ-constants.mjs`, `scripts/void-memory-organ-utils.mjs`, `scripts/void-memory-organ-vector.mjs`, `scripts/void-memory-organ-records.mjs`, `scripts/void-memory-organ-graph.mjs`, `scripts/void-memory-organ-cleanup.mjs`, `scripts/void-memory-organ-incubation.mjs`, `scripts/void-memory-organ-sleep.mjs`, `scripts/void-memory-organ-legacy-mirrors.mjs`, and `scripts/void-memory-organ-agency.mjs`, with `scripts/void-memory-organ-shared.mjs` as a temporary re-export shim.
- Most of the deterministic cleanup in `scripts/void-memory-organ-cleanup.mjs` is recent compensator cruft from failed attempts to manually clean an exploded state file. Do not preserve it as an earned organ. Architect the state boundary so that cleanup pile is unnecessary.
...
- First bounded move after the current commit: move memory-maintenance, incubation, sleep, and candidate-intervention outputs behind typed operations instead of direct canonical/working-projection writes.
