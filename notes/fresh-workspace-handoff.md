...
- `scripts/void-memory-organ.mjs` has started being split by concern. Shared limits/text sets, generic utilities, vector/embedder code, and memory-record intake now live in `scripts/void-memory-organ-constants.mjs`, `scripts/void-memory-organ-utils.mjs`, `scripts/void-memory-organ-vector.mjs`, and `scripts/void-memory-organ-records.mjs`, with `scripts/void-memory-organ-shared.mjs` as a temporary re-export shim.
