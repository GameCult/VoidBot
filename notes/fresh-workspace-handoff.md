...
- First bounded move after the current commit: let the enabled loop run under ordinary scheduling after the short-term clustering guard, then inspect the next unattended awake rumination and sleep cycle for repeated thought clusters, stale short-term residue, or ungrounded memory writes before calling the system broadly stable.
