...
- Sleep is now meant to be lossy. The nap pass in `scripts/void-memory-organ.mjs` promotes a few stronger seam summaries into semantic memory, then cuts raw episodic receipts, repo sweeps, archive excursions, novelty checks, and recent musings back down so awake rumination has to think from the seam instead of rereading yesterday's scrapbook.
...
- The next good bounded move is still public-lane hardening, especially around how the new discomfort/advocacy organs actually surface into room behavior, instead of another ceremonial anatomy lesson.
