...
- `scripts/void-memory-organ.mjs` is now a thin orchestrator. Shared limits/text sets, generic utilities, vector/embedder code, memory-record intake, graph/cluster synthesis, legacy translation, retention/compaction, incubation/bridge scoring, sleep/distillation, identity crystallization, candidate interventions, value-pressure inference, and legacy mirror repair live in focused `scripts/void-memory-organ-*.mjs` modules, with `scripts/void-memory-organ-shared.mjs` as a temporary re-export shim.
...
- First bounded move after the current commit: move memory-maintenance, incubation, sleep, identity, value-pressure, and candidate-intervention outputs behind typed operations instead of direct canonical/working-projection writes. Start with memory writes, because the schema now has a real meaning-preservation gate.
