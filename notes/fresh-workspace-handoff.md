...
- `scripts/run-void-moderator-rumination.ps1` has been rebuilt as a typed cursor-only runner. It polls recent Discord chronology, advances `void.moderation_cursor`, and does not launch Codex rumination, materialize JSON projection, post speech, or run legacy memory/agency behavior.
...
- The old 15-minute local scheduled task `Void Moderator Rumination` is disabled. Its replacement script is typed cursor-only and uses `.voidbot/private/void-self-state.cc`; it exists to keep chronology sane while the real typed phase machine is rebuilt, not to smuggle the old Codex rumination loop back in through the side door.
...
- First bounded move after the current commit: rebuild the scheduled moderation phase machine against typed `.cc` documents, then rebuild memory maintenance/sleep as typed operations with the existing meaning-preservation gate.
