...
- Landed agency-to-speech accountability: strong active self/world advocacy pressure now appears as `speechPressureObligations` in rumination context. The parent runner rejects silent `[]` unless the model queues/defers a candidate intervention or cools/retires the pressure. `npm run moderation:rumination:speech-pressure-fixture` proves both the muzzle-failure path and the deferred-candidate path.
