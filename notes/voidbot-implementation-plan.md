...
- Topic saturation and curiosity are one derived semantic surface, not stored truths. Repo Face turns now build a semantic curiosity graph from configured history/source vector retrieval plus current room/state/identity seeds, decode weighted attractor clusters, and hand the projector prominence, semantic saturation, novelty, recent-conversation overlap, Face-state overlap, density, jurisdiction fit, suggested motion, and evidence labels. The old repeated-term detector is removed rather than retained as fallback room weather.
...
- repo-scoped character turns reuse the typed self-state machinery per identity, but the child character must not see that machinery directly. Addressed chats are consumed by the heartbeat scheduler, which prioritizes the identity, and the intended boundary has two parent organs: `repo-face-state-projector.prompt.md` is Imagination for state/research/semantic thought clusters -> roleplay-friendly memory/mood/personhood prose, and `repo-face-turn-interpreter.prompt.md` is Mind for natural character reflection -> typed memory, affect, social-read, mood, agency, and speech operations. Current-room and nearby-channel logs are not model-summarized; deterministic prompt assembly attaches raw oldest-to-newest transcript evidence with correction/recency rules so the Face must read the human exchange itself. The same assembler also attaches compact room-weather facts so work-heavy/agent-heavy texture can become play/rest/social pressure without seeding fake banter. The state packet feeding Imagination is deliberately richer than before: private notes, values, activation vectors, runtime rest/speaking pressure, affect needs, bonds, status reads, mood dimensions, durable and short-term memory, agency pressure, incubating thoughts, candidate speech pressure, recent speech residue, social graph facts, room weather, and semantic curiosity attractors are all available for Imagination to turn into lived pressure. Face, Imagination, and Mind now default to capable `gpt-5.4` lanes instead of the Spark/Mini support chain. `scripts/run-repo-face-heartbeats.ts --assemble-prompt <identity>` composes the final child prompt from templates, channel policy, repo/archive research guidance, Bifrost digest, projected memory prose, deterministic conversation transcript, and room-weather facts. It must not turn typed state into character prose itself, and it should emit situational obligations only when the current packet proves them true.
