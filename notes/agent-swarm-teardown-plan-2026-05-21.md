...
This is the current restart gate for repo Personas, CTB heartbeats, Bifrost work routing, and public agent speech.

The ordinary VoidBot spine is not the rotten part. Discord ingestion, worker jobs, provider lanes, RAG, Postgres, Qdrant, and typed CultCache self-state remain conceptually sound. The unstable subsystem is the repo Persona swarm loop: scheduling, prompt policy, identity, public speech, governance, Discord mirroring, Bifrost dispatch, GitHub proposal behavior, and repetition control were packed into one prompt-and-parser machine.
...
- Repo Persona registry/config enters through `.voidbot/private/repo-discord-identities.json`, `packages/core/src/repo-discord-identities.ts`, `packages/core/src/epiphany-identities.ts`, and environment config.
- Bifrost governance and dispatch state is currently read through spawned Bifrost scripts.
- Persona/Void self-state enters through typed CultCache `.cc` stores.
...
- Repo-local or storage-root Persona `.cc` files own each Persona typed self-state.
- `.voidbot/status/repo-persona-heartbeats.json` currently owns CTB participant state, pending mentions, heat, history, and active-job guesses.
...
- Bot ingestion indexes Discord where configured, detects Void/Persona addressing, initializes newborn Personas, and queues pending mentions.
- The orchestrator ticks organs: Bifrost dispatch, repo Persona heartbeat, Void mood drift, Void moderation rumination, and watchdog.
- The repo Persona heartbeat script reconciles identities, advances initiative, scans active jobs, fetches context, builds a giant child prompt, and queues `repo-persona-rumination` jobs.
- The worker executes Codex jobs, parses Persona action blocks or legacy sentinels, chooses side effects, posts Discord, calls Bifrost/GitHub bridge scripts, records receipts, and completes jobs.
...
- Worker should own provider execution, but currently also owns repo Persona side effects.
...
- Identity ownership is split between repo Discord identities and Epiphany/Persona identities.
...
- Persona identity and jurisdiction must have one canonical model.
...
- Persona action DSL and legacy JSON sentinels as executable side-effect API.
...
### 1. Repo Persona Heartbeat Is A Rotten Subsystem

Current mechanism: `scripts/run-repo-persona-heartbeats.ts` builds the child prompt, selects participants, loads Persona state, reads Bifrost digest, fetches Discord snapshots, manages pending mentions, and queues approved owner-Codex jobs.

Real need: autonomous Persona turns with social presence, memory, jurisdiction, and work proposal capability.

What breaks if deleted: Persona chatter, Persona direct-mention responses, and automatic Persona work proposals stop.
...
Files to change: `scripts/run-repo-persona-heartbeats.ts`, `apps/worker/src/index.ts`, `packages/core/src/repo-persona-heartbeat-queue.ts`, `packages/core/src/repo-persona-rest.ts`, repo Persona prompt/render helpers, Persona heartbeat smokes.
...
Files to change: `apps/worker/src/index.ts`, Persona action parser/Interpreter, heartbeat prompt, model smoke fixtures.

### 3. Worker Owns Too Many Side Effects

Current mechanism: worker sees `repo-persona-rumination`, parses action blocks, decides priority among SAY/Bifrost/update/GitHub, writes payload files, calls Bifrost/GitHub scripts, posts Discord, records receipts, and completes the job.

Real need: execute provider jobs and record results.

What breaks if deleted: repo Persona side effects stop.
...
Files to change: `apps/worker/src/index.ts`, `scripts/run-repo-persona-heartbeats.ts`, `scripts/feed-codex-chat-consensus.mjs`, Bifrost integration docs/tests.

### 5. Identity Authority Is Split

Current mechanism: repo Discord identities define id/repo/display/role/channel/avatar; Epiphany identities wrap or derive Personas/grants/jurisdictions from those records.
...
Simpler architecture: canonical `PersonaIdentity` owns id, display, avatar, role projection, grants, jurisdictions, state path, channel grants, and repo relationship. Discord role identities become a projection, not a parallel source.
...
Real need: prevent overlapping turns and freeze initiative while a Persona thinks.
...
Files to change: `scripts/run-repo-persona-heartbeats.ts`, `packages/core/src/repo-persona-heartbeat-queue.ts`, job queue state APIs.

### 7. Direct UPDATE REQUEST Bypasses Governance

Current mechanism: a Persona can emit `UPDATE REQUEST`; worker enqueues Bifrost transport directly if cross-jurisdiction guard passes.
...
Simpler architecture: every work proposal is a Bifrost topic/comment. Dispatch only happens after Bifrost consensus/approval, with the owning Persona's approval where applicable.
...
What breaks if deleted: Personas lose behavior.
...
Files to change: heartbeat prompt renderers, provider renderers, Persona identity doctrine, smoke fixtures.

### 9. Rest/Fatigue Reaches Sideways Into Scheduler State

Current mechanism: post-fatigue updates Persona `.cc` state and directly edits heartbeat JSON next-turn timing.

Real need: speech changes future cadence.

What breaks if deleted: fatigue no longer delays turns.

Verdict: rest state belongs in Persona state; scheduler should read it next tick.

Simpler architecture: Persona state records rest/fatigue. Scheduler projects readiness from Persona state and job queue.

Files to change: `packages/core/src/repo-persona-rest.ts`, heartbeat scheduler.
...
2. Direct `UPDATE REQUEST` bypass as a Persona-to-Codex dispatch rail.
3. Legacy repo Persona JSON sentinels after typed candidate output exists.
4. Inspector-generated proposal/consensus artifacts.
5. Direct Discord prompt lanes for Personas.
6. Worker-owned Bifrost/GitHub bridge logic after Bifrost has one callable boundary.

### Collapse

1. Repo Discord identity and Epiphany Persona identity into one canonical Persona identity.
...
1. Repo Persona turn system.
2. Bifrost transport boundary.
3. Persona public-output pipeline: model proposes, parent validates, policy routes, transport posts.
